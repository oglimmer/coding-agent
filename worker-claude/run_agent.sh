#!/usr/bin/env bash
# coding-agent worker pipeline (Claude Code / DeepSeek variant):
...
#   repo's GitHub Action review -> fix any findings with Claude Code -> re-review
#   -> squash-merge -> emit result line.
#
# This is the aider worker's twin: the GitHub orchestration (clone, verify gate,
# self-review judge, PR/review/merge loop, result marker) is identical, but the
# coding engine is Anthropic's Claude Code CLI pointed at a DeepSeek backend
# (https://api-docs.deepseek.com/quick_start/agent_integrations/claude_code/)
# instead of aider. Claude Code explores the repo, edits files and runs tests
# itself, so the aider-only machinery (scoping, test-command detection, the
# architect/editor split, --auto-test, per-model sampling knobs) is gone.
#
# Required env:
...
#   DEEPSEEK_API_KEY    DeepSeek key — authenticates Claude Code's DeepSeek
#                       backend (as ANTHROPIC_AUTH_TOKEN) AND the helper calls
#                       below (self-review / review-judge on chat/completions)
# Optional env:
...
#   VERIFY_TIMEOUT      seconds for the authoritative verify gate (default: 1200)
set -uo pipefail
...
# review. We classify that prose with a model before merging, using the same
...
# --- verify-gate infrastructure ------------------------------------------------
# Shared with the aider worker: wrap the repo's (possibly compound, multi-`cd`)
# verify command into a bounded, non-interactive runner so a hang degrades to a
# failed round instead of a dead job, and install the deps every stack it touches.

# Wrap a raw command into a bounded, CI-mode runner and echo the runner command.
# CI=1/CI=true flips vitest/jest/react-scripts out of watch mode; `timeout` kills
# a genuinely wedged process so a hang becomes "this round failed", not a dead job.
BOUNDED_SEQ=0
bounded_runner() {
...
# package.json so a misconfigured verify command fails with a clear reason.
check_npm_script() {
...
# exporting it onto PATH so the verify command's subprocess uses it.
TARGET_VENV=""
prepare_python() {
  local dir="$1"
...
# change, and prepare each stack the command actually touches (both stacks of a
# cross-stack "cd backend && go test && cd ../frontend && npm run test").
prepare_cmd_deps() {
...
# The repo-configured verify command is the authoritative pre-PR gate, but if it
...
# Files touched on the branch so far (excluding deletions) — used for context in
# the self-review/verify findings prompts.
branch_files() {
...
make sure the project still builds and its tests pass.
EOF
...
# back to Claude for a corrective round. Fails OPEN after SELF_REVIEW_ROUNDS.
self_review() {
  SELF_OK="no"
  SELF_CRITIQUE=""
...
# Install node deps for every tracked package.json so the repo's pre-commit hooks
# (eslint/tsc/vitest) don't fail with "command not found" (exit 127).
...
# even then the review body is just the marker — so we key off the sticky comment.
